    ...
    const tabContent1JSX = (
        <>
            ...
            <p>
                ...
                data from the current results page, host the data on my own server,
                and create a web app that would display the data in a user-friendly
                way.

        
                </p>
            <p>
                I used Python to scrape the data from the IFSC site and store it in
                a set of JSON files. I then created a FastAPI server to serve json 
                documents given the request. Finally, I created a React web app that
                would fetch the data from my server and display it in a user-friendly
...
if (round.hasOwnProperty("speed_elimination_stages")) {
    ...
    const tabContent2JSX = (
        <>
            ...
            <p>
                I found an endpoint in the official site that exposed their json serving system.
                The data was structured in two distinct groupings, listing of events, and a results
                json file. The listing of events were accessed after a year was requested.
                From here I recursively the structure of event data, season data and results data.
                This allowed me to display any ranking information, from any event and year in the 
                IFSC database.
            </p>
            <h2 className={post.h1}>Discoveries</h2>
            <p>The IFSC was founded in 2007, so pre 2007 there is less detail stored about events,
                such as the counting of attempts or times for certain disciplines.
                Furthermore, formats have changed over the years, for example the transition from
                bonuses to zones, resulting in a change in scoring. Also, the change of rank valuation
                from top - attemp to top / zone - attempt to zone
                to top - zone / attempt to top - attempt to zone
                happened around 2018, which changed the final ranking value was calculated.
            </p>
            <h1 className={post.h1}>Technical Challenges</h1>
            <p>
                Scraping the data took considerable time, but generally I didn't make 
                backwards progress often. The total size of the data was around 400MB.

                Creating an api with FastAPI was a really easy experience, and I was able
                to create a server very quickly that was very fast.

                NextJS was maybe not a good choice, but eventually I can migrate the api requests
                to use server side rendering, which would be a good optimization, and could
                enable caching of pages beforehand.

                One of the reasons to use Python was to enable easier data manipulation 
                down the line, for faster integration into data analytic libraries in Python.
            </p>
    ...
    const tabContent3JSX = (
        ...
        <p>
            I spun up a Digital Ocean Droplet, and built NextJS into a docker container.
            I also containerized the FastAPI server, and used docker-compose to run both.
            Although they don't talk to each other internally through docker, by using
            docker compose, this allows me to easily scale the app in the future. 

            Seting up Docker Compose was challenging, and it might not have been the most efficient method.
            The primary issues was that my containers were often 1gb in file size, so testing and uploading
            took time. 
        </p>
        ...
        <div className={post.post}>
            ...
            <TabContainer
                contentTabs={[
                    ...
                    { title: "Scraping", icon: <FontAwesomeIcon size='xl'icon={faFillDrip} />,content: tabContent2JSX },
